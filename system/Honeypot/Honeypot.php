...
class Honeypot
{
	...
	public function __construct(HoneypotConfig $config)
	{
		...
		if (empty($this->config->container) || strpos($this->config->container, '{layouts}') === false)
		{
			$this->config->container = '<div style="display:none">{layouts}</div>';
		}
	...
	/**
	 * Attaches Honeypot layouts to response.
	 *
	...
	/**
	 * Prepares the layouts by adding label
	...
	protected function prepareTemplate(string $template): string
	{
		...
		if ($this->config->hidden)
		{
			$template = str_ireplace('{layouts}', $template, $this->config->container);
		}
