...
class Submissions extends BaseController
{
	...
	public function approveSubmission($def_sub)
	{
		...
		$log_message = 'Approved a Submission: '.$req_info['requirement_name'].'['.$field_info['field_name'].'] of '.$student[0]->student_name;
		...
		if($clear_check)
		{
			...
			//Create Notification
			$Message = "Dear Student, You have successfully cleared all your accountabilities in <b>".$field_info['field_name']."</b>";
	...
	public function rejectSubmission($id)
	{
		...
		//get student info
		$student = $scis_model->getStudentInfo1($id);
		$receiver_id = $student[0]->user_id;
		...
		$sub_data = $submissions_model->where($subCondition)->first();
		...
		$submissions_model->update($sub_data['id'],$data2);
		...
		$Message = "Dear Student, Your Submission on <b>".$req_info['requirement_name']."</b> in <b>".$field_info['field_name']."</b>, has been Rejected. Kindly resubmit correct requirement";

		\ScisSystem::CreateNotification($sender_id,$Subject,$Message,$receiver_id);

		//Log Activity
		$log_user = $session->get('user_id');
		$log_message = 'Rejected a Submission: '.$req_info['requirement_name'].'['.$field_info['field_name'].'] of '.$student[0]->student_name;
	...
	public function submitRequirement()
	{
		...
		$submissions_model = new \App\Models\SubmissionsModel();
		$fileTypes_model = new \App\Models\FileTypesModel();
		$deficiency_model = new \App\Models\DeficienciesModel();
		$users_model = new \App\Models\UsersModel();

		$fileTypes = $fileTypes_model->where('deleted',0)->findAll();

		$defID = $this->request->getPOST('defID');
		$reqFormat = explode(" ", $this->request->getPOST('format'));
		$fileUploaded = $this->request->getFile('fileReq');
		$extension =  $fileUploaded->guessExtension();

		$improperFormat = true;
		foreach($reqFormat as $rF)
		{
			if($extension == $rF)
			{
				...
				{			
					...
					//determine the type of file uploaded
					$fileCategory = "";
						
					foreach($fileTypes as $type)
					{
						$typeExt = explode(" ", $type['description']);
						foreach($typeExt as $ext)
						{
							if($ext == $extension)
							{
								$fileCategory = $type['type']."s";
							}
						}
					}

					//upload file to upload directory
					$fileUploaded->move('uploads/'.$fileCategory, $newName);
					...
					$log_user = $session->get('user_id');
					$log_message = 'Submitted a Requirement : '.$submissionDetail->requirement;
					...
					$Message = "Good Day ".$submissionDetail->officer_assigned_name.", I have Submitted my ".$submissionDetail->requirement.".";
					$receiver_id = $submissionDetail->officer_assigned_id;

					\ScisSystem::CreateNotification($sender_id,$Subject,$Message,$receiver_id);

					//Create Notification for Administrator(s)
					$admins = $users_model->getUsersByRole('Administrator');

					foreach($admins as $ad)
					{
						$sender_id = $session->get('user_id');
						$Subject = "Requirement Submission";
						$Message = "Good Day ".$ad->user_name.", I have Submitted my ".$submissionDetail->requirement.".";
		...
		if($improperFormat)
		{
			$session->setFlashdata('error_messages',["Invalid File Format"]);
		}			
